...
class FlexibleProgressBar:
    def __init__(self, config: Dict[str, Any]):
        ...
        # 角色配置
        self.character_path = config.get('character_path', 'assets/characters/pikaqiu.gif')
    ...
    def _draw_bar_background(self, frame: np.ndarray, bar_x: int, bar_y: int, 
                          bar_width: int, bar_height: int) -> np.ndarray:
        """绘制进度条背景和边框"""
        ...
        if self.border_thickness > 0:
            cv2.rectangle(frame_copy, 
                         ...
                         (bar_x + bar_width + self.border_thickness, bar_y + bar_height + self.border_thickness),
    ...
    def _draw_progress_fill(self, frame: np.ndarray, bar_x: int, bar_y: int, 
                          bar_width: int, bar_height: int, progress: float) -> np.ndarray:
        """绘制进度填充"""
        ...
        if self.gradient_enabled:
            ...
            for i in range(progress_width):
                ...
                cv2.line(frame, (bar_x + i, bar_y), 
                        (bar_x + i, bar_y + bar_height), color, 1)
        else:
            # 纯色进度条
        ...
        if self.glow_enabled and self.frame_count % 20 < 10:
            ...
            if shine_x > bar_x:
                cv2.line(frame, (shine_x, bar_y), 
                        (shine_x, bar_y + bar_height), (255, 255, 255), 3)
        
        return frame

    def _draw_character(self, frame: np.ndarray, bar_x: int, bar_y: int, 
                       bar_width: int, progress: float, width: int, height: int) -> np.ndarray:
        """绘制角色"""
        ...
        # 绘制特效
        self._draw_lightning_effects(frame)
        self._draw_trail_particles(frame)
        ...
        try:
            frame = self._blend_with_alpha(frame, current_char, current_alpha, char_x, char_y)
        except Exception as e:
            print(f"绘制角色时出错: {e}")
        
        return frame

    def _draw_progress_text(self, frame: np.ndarray, bar_x: int, bar_y: int, 
                          bar_width: int, progress: float) -> np.ndarray:
        """绘制进度文字"""
    ...
    def process_video(self, input_video: str, output_video: str = None) -> str:
        """处理视频，添加进度条并保留音频"""
...
配置示例:
  ...
  # 配置文件 + 命令行参数覆盖
  python video_progress.py --config config.json --size 80 80
  ...
  python video_progress.py video.mp4 --character assets/characters/panda_running.gif
    ...
    try:
        progress_bar = FlexibleProgressBar(config)
